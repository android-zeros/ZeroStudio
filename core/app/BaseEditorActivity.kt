...
import android.zero.studio.lsp.service.LspLanguageServerService
import android.zero.studio.lsp.service.WorkspaceSymbolService
...
abstract class BaseEditorActivity : EdgeToEdgeIDEActivity(), TabLayout.OnTabSelectedListener,
  ...
  @Subscribe(threadMode = MAIN)
  open fun onDiagnosticResult(result: DiagnosticResult) {
      if (isDestroying) return
      
      val error = result.diagnostics.isEmpty()
      handleDiagnosticsResultVisibility(error)
      if (error) return

      val diagnosticsMap = mapOf(result.file.toFile() to result.diagnostics)
      val groups = DiagnosticUtil.mapAsGroup(diagnosticsMap)
      setDiagnosticsAdapter(DiagnosticsAdapter(groups, this))
  }
  ...
  private fun startLspServices() {
    log.i("Starting all LSP background services...")
    try {
      startService(Intent(this, LspLanguageServerService::class.java))
      startService(Intent(this, WorkspaceSymbolService::class.java))
    } catch (e: Exception) {
      log.e("Failed to start one or more LSP services", e)
    }
  }

  private fun stopLspServices() {
    log.i("Stopping all LSP background services...")
    try {
      stopService(Intent(this, LspLanguageServerService::class.java))
      stopService(Intent(this, WorkspaceSymbolService::class.java))
    } catch (e: Exception) {
      log.e("Failed to stop one or more LSP services", e)
    }
  }
  ...
  override fun onTabReselected(tab: Tab) {
    createMenu(this, tab.view, com.itsaky.androidide.actions.ActionItem.Location.EDITOR_FILE_TABS, true).show()
  }
