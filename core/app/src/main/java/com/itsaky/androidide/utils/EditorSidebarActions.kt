        ...
        (railView.menuView as NavigationBarMenuView)
          ...
          ?.setOnLongClickListener {
            ...
            val args = Bundle().apply {
                ...
                // 2. 关键点：将 FAILSAFE 设为 true
                putBoolean(TERMUX_ACTIVITY.EXTRA_FAILSAFE_SESSION, true)
            }
            
            controller.navigate(TerminalSidebarAction.ID.hashCode(), args, navOptions {
                launchSingleTop = true
                restoreState = true
            })
            
            // 手动更新 UI 状态
            val item = railView.menu.findItem(TerminalSidebarAction.ID.hashCode())
            if(item != null) {
                item.isChecked = true
                titleRef.get()?.text = item.title
            }
