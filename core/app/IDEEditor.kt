...
import android.zero.studio.lsp.window.LspEditorTextActionWindow
...
open class IDEEditor @JvmOverloads constructor(
  ...
  private val editorFeatures: EditorFeatures = EditorFeatures()
    ...
    /**
     * The wrapper for Sora Editor's LSP functionality.
     * It is initialized and managed by [LspManager].
     */
  ...
  fun setFile(file: File?) {
    ...
    file?.also {
        ...
        setupLanguage(it) // Setup TreeSitter/ANTLR syntax highlighting first

        // Attach to LSP Manager which will handle connection and LspEditor creation
        val projectPath = ProjectManagerImpl.getInstance().projectDirPath
        if (projectPath.isNotEmpty()) {
            LspManager.attachEditor(context, this, it, projectPath, editorScope) { lspEditor ->
                setupLspFeatures(lspEditor)
            }
        }
    }
  }
    ...
    /**
     * Called by LspManager after the LspEditor instance is created and connected.
     * This is where we set up LSP-specific features.
     * @author android_zero
     */
    internal fun setupLspFeatures(lspEditor: LspEditor) {
        this.lspEditor = lspEditor
        
        // Replace default text action window with the LSP-aware one
        val lspActionWindow = LspEditorTextActionWindow(lspEditor)
        lspActionWindow.setOnMoreButtonClickListener { _, editor ->
            editor.coroutineScope.launch {
                editor.eventManager.emitAsync(EventType.codeAction) {
                    put(this@IDEEditor.cursor.range.asLspRange())
                }
            }
        }
        replaceComponent(EditorTextActionWindow::class.java, lspActionWindow)
        
        // Listen for diagnostics from sora-lsp and bridge them to AndroidIDE's UI
        lspEditor.eventManager.addEventListener(object : EventListener {
            override val eventName: String = PublishDiagnosticsEvent().eventName
            ...
            override suspend fun handleAsync(context: EventContext) {
                ...
                val file = this@IDEEditor.file ?: return
                
                val diagnosticItems = diagnostics.transformToEditorDiagnostics(this@IDEEditor).mapNotNull {
                    it.detail as? com.itsaky.androidide.lsp.models.DiagnosticItem
                }
                
                // Post an event for the activity to update the bottom sheet
                EventBus.getDefault().post(DiagnosticResult(file.toPath(), diagnosticItems))
            }
        })
    }
...
fun executeCommand(command: Command?) {
  ...
  fun findDefinition() {
    ...
    lspEditor?.eventManager?.emitAsync(EventType.definition, cursor.left())
  }

  fun findReferences() {
    if (isReleased) return
    lspEditor?.eventManager?.emitAsync(EventType.references, cursor.left())
  }

  fun expandSelection() {
    log.warn("Expand Selection is not implemented via LSP yet.")
  }

  override fun ensureWindowsDismissed() {
    lspEditor?.codeActionWindow?.dismiss()
    lspEditor?.hoverWindow?.dismiss()
    lspEditor?.signatureHelpWindow?.dismiss()
  ...
  override fun release() {
    ...
    // Detach from LSP Manager before releasing other resources
    LspManager.detachEditor(this)
    lspEditor = null
  ...
  open fun setupLanguage(file: File?) {
    ...
    createLanguage(file) { language ->
      ...
      // Update wrapper language in LspEditor
      lspEditor?.wrapperLanguage = this.editorLanguage
    }
  }
  ...
  protected open fun initEditor() {
    ...
    // Disable the default one. It will be replaced by LspEditorTextActionWindow upon LSP connection.
