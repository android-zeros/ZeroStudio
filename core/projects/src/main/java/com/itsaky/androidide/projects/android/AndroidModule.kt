 ...
 * @param variants The list of build variants.
 * @param configuredVariant The variant that is currently selected/configured.
 * @param cppProjectMetadata Metadata for C++ (NDK) components, if any.
 * @param classesJar The compiled classes jar for this module.
 *
 * @author Akash Yadav
 * @author android_zero
 */
open class AndroidModule(
  ...
  val cppProjectMetadata: CppProjectMetadata?,
  val classesJar: File?
) : ModuleProject(
  ...
  fun getResourceDirectories(): Set<File> {
    ...
    // }
    mainSourceSet.sourceProvider?.resDirectories?.let { dirs.addAll(it) }
  ...
  override fun getSourceDirectories(): Set<File> {
    ...
    // src/main/java
    val sources = mainSourceSet.sourceProvider?.javaDirectories?.toMutableSet() ?: mutableSetOf()

    // src/main/kotlin
    sources.addAll(mainSourceSet.sourceProvider?.kotlinDirectories ?: emptyList())
