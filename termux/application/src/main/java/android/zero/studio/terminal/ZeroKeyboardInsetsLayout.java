 ...
 *
 * FIXED FOR FRAGMENT EMBEDDING:
 * 1. Top Padding is forced to 0. Since this Fragment is embedded in an Activity that 
 *    already handles the Status Bar, adding top padding here causes a double gap.
 * 2. Bottom Padding logic handles the Navigation Bar vs Keyboard conflict to prevent 
 *    white strips or UI flying off.
 *
 * @author android_zero
 */
public class ZeroKeyboardInsetsLayout extends FrameLayout {
    ...
    public WindowInsetsCompat onApplyWindowInsets(WindowInsetsCompat insets) {
        // 1. Get System Bars (Status Bar + Navigation Bar)
        ...
        // 2. Check if Keyboard (IME) is visible
