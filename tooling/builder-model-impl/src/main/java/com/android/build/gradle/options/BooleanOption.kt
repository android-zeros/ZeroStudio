...
// import com.android.builder.model.PROPERTY_AVOID_TASK_REGISTRATION
 ...
 * @author android_zero
 *
 * AGP 9.0 UPG: Synced with AGP 9.0.0-alpha14.
 * - REMOVED: USE_ANDROID_X, ENABLE_JETIFIER.
 * - UPDATED: Deprecation targets for BUILD_FEATURE flags.
 * - ADDED: New experimental flags like LINT_USE_K2_UAST, ENABLE_JACOCO_AGENT_TRANSFORM, etc.
 * - PROMOTED: Several flags moved from experimental to softly-enforced for AGP 9.0.
 */
enum class BooleanOption(
     ...
     * (e.g. `com.android.build.gradle.internal.publishing.AndroidArtifacts.ArtifactScope.EXTERNAL`)
    ...
    ENABLE_INSTRUMENTATION_TEST_DESUGARING("android.experimental.library.desugarAndroidTest", false, ApiStage.Experimental),
    ...
    ENABLE_TEST_PROFILING("android.experimental.enableTestProfiling", false, FeatureStage.Experimental),
    /**
    ...
    ENABLE_JACOCO_AGENT_TRANSFORM(
        "android.experimental.enableJacocoAgentTransform",
        false,
        FeatureStage.Experimental,
    ),
    USE_NEW_LINT_MODEL_READER(
        "android.experimental.useNewLintModelReader",
        false,
        FeatureStage.Experimental,
    ),

    AVOID_TASK_REGISTRATION_ON_CONFIGURATION_CACHE(
        PROPERTY_AVOID_TASK_REGISTRATION,
        false,
        ApiStage.Experimental,
    ),

    GENERATE_R_CLASS_SOURCES(
        "android.generateRClassSources",
        true,
        FeatureStage.Experimental,
    ),

    ENABLE_COMPILE_CLASSPATH_ALIGNMENT(
        "android.experimental.enableCompileClasspathAlignment",
        true,
        FeatureStage.Experimental,
    ),

    ENABLE_SDK_INFORMATION_ARTIFACT(
        "android.experimental.enableSdkInformationArtifact",
        false,
        FeatureStage.Experimental,
    ),

    GENERATE_SOURCE_DEBUG_SYMBOLS(
        "android.experimental.generateSourceDebugSymbols",
        true,
        FeatureStage.Experimental,
    ),

    ENABLE_PRIVACY_SANDBOX_FOR_LIBRARIES(
        "android.experimental.enablePrivacySandboxForLibs",
        false,
        FeatureStage.Experimental,
    ),
    ...
    /**
     * Whether to compile custom lint checks against the same lint libraries that AGP uses (true by
     * default).
     */
    USE_LINT_COMMON_LIBRARIES(
        "android.experimental.useLintCommonLibraries",
        true,
        FeatureStage.SoftlyEnforced(VERSION_9_0)
    ),
