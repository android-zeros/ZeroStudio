...
import com.itsaky.androidide.tooling.api.messages.TestExecutionMessage
...
internal class ToolingApiServerImpl(private val project: ProjectImpl) :
  ...
  override fun executeTests(message: TestExecutionMessage): CompletableFuture<TaskExecutionResult> {
    return runBuild {
      if (!isServerInitialized().get()) {
        log.error("Cannot execute tests: {}", PROJECT_NOT_INITIALIZED)
        return@runBuild TaskExecutionResult(false, PROJECT_NOT_INITIALIZED)
      }
      
      log.debug("Received request to run tests: {}", message)

      Main.checkGradleWrapper()

      val connection = checkNotNull(this.connection) {
        "ProjectConnection has not been initialized. Cannot execute tests."
      }

      val launcher: TestLauncher = connection.newTestLauncher()

      // 配置要执行的任务
      if (message.tasks.isNotEmpty()) {
        launcher.forTasks(*message.tasks.toTypedArray())
      }
      // 配置要执行的类和方法
      val classesToRun = mutableSetOf<String>()
      val methodsToRun = mutableMapOf<String, MutableSet<String>>()
      
      message.jvmTests.forEach { spec ->
        val methodName = spec.methodName
        if (methodName != null) {
          methodsToRun.computeIfAbsent(spec.className) { mutableSetOf() }.add(methodName)
        } else {
          classesToRun.add(spec.className)
        }
      }

      if (classesToRun.isNotEmpty()) {
        launcher.withJvmTestClasses(classesToRun)
      }
      methodsToRun.forEach { (className, methods) ->
        launcher.withJvmTestMethods(className, methods)
      }
      
      // 配置调试模式
      if (message.debugPort > 0) {
        launcher.debugTestsOn(message.debugPort)
      }
     
     // 配置通用启动器参数
      this.buildCancellationToken = GradleConnector.newCancellationTokenSource()
      Main.finalizeLauncher(launcher)
      launcher.withCancellationToken(this.buildCancellationToken!!.token())

      val tasksDisplayName = if (message.tasks.isNotEmpty()) message.tasks else listOf("tests")
      notifyBeforeBuild(BuildInfo(tasksDisplayName))

      try {
        launcher.run()
        this.buildCancellationToken = null
        notifyBuildSuccess(tasksDisplayName)
        return@runBuild TaskExecutionResult.SUCCESS
      } catch (error: Throwable) {
        notifyBuildFailure(tasksDisplayName)
        return@runBuild TaskExecutionResult(false, getTaskFailureType(error))
      }
    }
  }
