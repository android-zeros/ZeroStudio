 ...
 * @author android_zero
 * 
 * AGP 9.0 UPG: Updated to fetch the new `AndroidDsl` model.
 * Adapted the `VariantDependencies` request to use the new map-based `dontBuildHostTestRuntimeClasspath` parameter.
...
class AndroidProjectModelBuilder(initializationParams: InitializeProjectParams) :
  ...
  override fun build(param: AndroidProjectModelBuilderParams): IAndroidProject {
    ...
    return AndroidProjectImpl(
      ...
      versions,
      androidDsl,
      null // CppProjectMetadata is passed later in ModuleProjectModelBuilder
    )
  }
}