 ...
 * @author android_zero
 * 
 * AGP 9.0 UPG: Updated `withRuntimeConfiguration` to handle the new `ApplicationVariantImpl` class,
 * ensuring compatibility with AGP 9.0's internal API changes.
...
class LogSenderPlugin : Plugin<Project> {
  ...
  private fun ApplicationVariant.withRuntimeConfiguration(
    ...
    // AGP 9.0 changed the internal implementation of ApplicationVariant
    when (this) {
        is ApplicationVariantImpl -> this.variantDependencies.runtimeClasspath.action()
        is AnalyticsEnabledApplicationVariant -> this.delegate.withRuntimeConfiguration(action)
    }
  }
}