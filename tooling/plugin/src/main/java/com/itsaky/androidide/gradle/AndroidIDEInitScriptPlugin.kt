 ...
 * @author android_zero
 * 
 * AGP 9.0 UPG: Refactored repository configuration to be more explicit within the
 * buildscript block, removing the now-unnecessary `settingsEvaluated` callback.
...
class AndroidIDEInitScriptPlugin : Plugin<Gradle> {
  ...
  private fun getTestEnvProps(startParameter: StartParameter): Pair<Boolean, String> {
    return startParameter.run {
      ...
      val mavenLocalRepos = projectProperties.getOrDefault(_PROPERTY_MAVEN_LOCAL_REPOSITORY, "").toString()
