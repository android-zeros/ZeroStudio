 ...
 * - drawRowRegionBackground (removed color parameter)
 */
...
class TracingEditorRenderer(
  ...
  /**
   * FIX: The signature of `drawMiniGraph` in the superclass has changed.
   * The `graph` parameter is now a `Drawable?` instead of a `String?`.
   */
  ...
  /**
   * FIX: The signature of `drawRowRegionBackground` in the superclass has changed.
   * The `color` parameter has been removed.
   */
  override fun drawRowRegionBackground(canvas: Canvas?, row: Int, line: Int, highlightStart: Int,
    highlightEnd: Int, rowStart: Int, rowEnd: Int) = trace("drawRowRegionBackground") {
